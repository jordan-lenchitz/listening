...
class TrackerConfig:
    ...
    # Tracking parameters
    max_pitch_jump_cents: float = (
        300  # Maximum allowed pitch change per frame (~3 semitones)
    )
    ...
    combination_tone_tolerance_cents: float = (
        30  # Tolerance for matching combination tones
    )
...
class MultiF0Tracker:
    ...
    def _compute_salience(self, spectrum: np.ndarray, freqs: np.ndarray) -> tuple:
        ...
        f0_candidates = np.logspace(
            np.log10(self.config.min_freq), np.log10(self.config.max_freq), n_bins
        )
    ...
    def _detect_peaks(self, freqs: np.ndarray, salience: np.ndarray) -> list:
        ...
        peaks = []
        min_dist_bins = int(
            self.config.min_peak_distance_cents / (1200 * np.log2(freqs[1] / freqs[0]))
        )  # Convert cents to bins
            ...
            if 0 < idx < len(salience) - 1:
                ...
                if beta > 0:
                    ...
                    refined_freq = freqs[idx] * (
                        2 ** (offset * np.log2(freqs[1] / freqs[0]))
                    )
        ...
        # Limit to max_voices
        return peaks[
            : self.config.max_voices * 2
        ]  # Keep extra for combination tone detection
    ...
    def _identify_combination_tones(
        self, sung_pitches: list, detected_pitches: list
    ) -> list:
        """
    ...
    def _update_tracks(self, detected_pitches: list, frame: int):
        ...
        if not active_tracks:
            ...
            for pitch, salience in detected_pitches[: self.config.max_voices]:
                if salience > self.config.peak_threshold:
                    track = VoiceTrack(id=self.next_track_id, start_frame=frame)
        ...
        if self.config.detect_extra_pitches and sung_pitches:
            ...
            combo_tones = self._identify_combination_tones(
                sung_pitches, [p for _, p in unassigned]
            )
            ...
            for j, (pitch, salience) in unassigned:
                if pitch in combo_freqs:
                    ...
                    track = VoiceTrack(
                        id=self.next_track_id, start_frame=frame, is_extra_pitch=True
                    )
                    track.add_observation(frame, pitch, salience)
                    track.state = VoiceState.ACTIVE
                    self.tracks.append(track)
    ...
    def track(self, audio: np.ndarray, sr: int) -> dict:
        ...
        times = librosa.frames_to_time(
            np.arange(n_frames), sr=sr, hop_length=self.config.hop_length
        )
        ...
        extra_pitches = [t for t in self.tracks if t.is_extra_pitch and t.duration >= 2]
...
def visualize_tracks(result: dict, output_path: Optional[str] = None, show: bool = True):
    """
    ...
    for track in extra_pitches:
        ...
        ax.plot(
            track_times, track_pitches, "--", color="purple", linewidth=2, alpha=0.7
        )
        ax.plot(
            track_times[0],
            track_pitches[0],
...
def analyze_audio_file(audio_path: str, config: Optional[TrackerConfig] = None) -> dict:
    """
